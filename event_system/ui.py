...
class EventManagementScreen(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(form_frame, text="Venue:").grid(row=2, column=0, sticky="w", pady=2)
        self.venue_entry = tk.Entry(form_frame)
        self.venue_entry.grid(row=2, column=1, sticky="ew", pady=2)

        tk.Label(form_frame, text="Total Slots:").grid(row=3, column=0, sticky="w", pady=2)
        self.total_slots_entry = tk.Entry(form_frame)
        self.total_slots_entry.grid(row=3, column=1, sticky="ew", pady=2)
        ...
        self.tree = ttk.Treeview(list_frame, columns=("ID", "Name", "Date", "Venue", "Slots"), show='headings')
    ...
    def handle_create_event(self):
        ...
        # --- Validation ---
        if not all([name, date_str, venue, slots_str]):
        ...
        # --- Create Event ---
        success = events.create_event(name, event_date, venue, total_slots)
        if success:
    ...
    def populate_events_list(self):
        ...
        for event in all_events:
            ...
            event_id, name, date, venue, slots = event
            formatted_date = date.strftime("%Y-%m-%d")
            self.tree.insert("", "end", values=(event_id, name, formatted_date, venue, slots))
...
class EmailNotificationScreen(tk.Frame):
    ...
    def handle_send_emails(self):
        ...
        # event_details tuple: (event_id, event_name, event_date, venue, total_slots)
        _, event_name, event_date, venue, _ = event_details
        ...
        email_body = email_utils.create_event_notification_email_body(
            ...
            "N/A", # Time is not stored in DB currently, can be added if needed
            venue,
            custom_message
        )
...
class RegistrationScreen(tk.Frame):
    ...
    def handle_event_selection(self, event):
...
class AttendanceScreen(tk.Frame):
    ...
    def handle_event_selection(self, event):
        """Updates the attendance list when an event is selected."""
