...
3.  **Configure the Application:**
    *   Open the `event_system/config.py` file.
    *   Update the `DB_CONFIG` dictionary with your Oracle username, password, and connection string (DSN).
    *   Update the `EMAIL_CONFIG` dictionary with your SMTP server details for sending emails.

    ```python
    # event_system/config.py
    DB_CONFIG = {
        'user': 'your_username',
        'password': 'your_password',
        'dsn': 'your_host:your_port/your_service_name'  # e.g., 'localhost:1521/orcl'
    }
